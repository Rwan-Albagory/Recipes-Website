...
async function submitFormData(email, password, phone, name, retypePassword) {
  ...
  const trimmedEmail = email.trim(); // إضافة هذا للتحقق من عدم وجود مسافات
  ...
  console.log({
   ...
   // ��ضافة هذا للتحقق من عدم وجود مسافات
  });
  ...
  try {
    const response = await fetch(
      "https://ecommerce.routemisr.com/api/v1/auth/signup",
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json", // إضافة الهيدر
        },
        body: JSON.stringify({
